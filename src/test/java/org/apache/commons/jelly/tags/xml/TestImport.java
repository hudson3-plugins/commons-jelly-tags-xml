...
public class TestImport extends TestCase {
    ...
    // Note that the original test expected two xmlns attributes!
    // Some implementations apparently don't remove duplicate attributes.
    // This is not an error, though it's peculiar.
    private String expected = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
        + "<html xmlns=\"http://www.w3.org/TR/xhtml1/strict\">"
        // This bogus test expected two xmlns attributes! BF
        //+ "xmlns=\"http://www.w3.org/TR/xhtml1/strict\">"
        + "<head><title>Expense Report Summary</title></head>"
        + "<body><p>Total Amount: 12</p></body></html>";
    private String expected2 = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
        + "<html xmlns=\"http://www.w3.org/TR/xhtml1/strict\" "
        + "xmlns=\"http://www.w3.org/TR/xhtml1/strict\">"
        + "<head><title>Expense Report Summary</title></head>"
        + "<body><p>Total Amount: 12</p></body></html>";
...
//         of the URL
        ...
        out.close();
        String was = writer.toString();
        if (DEBUG) debugValues(expected2, was);
        assertTrue("Got expected value", expected.equals(was) || expected2.equals(was));
    ...
    private void debugValues(String expected, String was) {
      int elen = expected.length();
      int wlen = was.length();
      if (elen != wlen)
        System.out.println("**Expected length "+elen+" was length "+wlen);
      int min = Math.min(elen, wlen);
      int i = 0;
      for (; i < min; i++) {
        char ec = expected.charAt(i);
        char wc = was.charAt(i);
        if (ec != wc) {
          System.out.println("**First difference at index "+i+" expected='"+ec+"' was='"+wc+"'");
          return;
        }
      }
      if (elen == wlen)
        ...
        System.out.println("**Strings equal up to "+(elen < wlen ? "expected" : was)+" length");
    }
